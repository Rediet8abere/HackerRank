# My solution in Python, using back-tracking and recursion. Tried to use understandable function names.
    ...
    for i in range(10):
        for j in range(10 - length + 1):
            ...
            for k in range(len(word)):
                if list(board[i])[j + k] not in ['-', word[k]]:
                    good = False
                    break
            if good:
...
def solve(board, words):
    ...
    print("word: ", word, "pos_locs", pos_locs)
    for loc in pos_locs:
        move(board, word, loc)
        solve(board, words)
